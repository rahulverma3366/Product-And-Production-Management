...
switch ($action) {
	case 'submit':
        ...
        foreach ($s_ids as $val) {
            ...
            $qty = $_REQUEST['qty'][$i];
            $cdata = $db->query("SELECT * FROM `imei_update` WHERE consumer_id = '$consumer_id' AND p_id = '$product_id' AND sold = 0");
	...
	case 'another_method':
        ...
        foreach ($s_ids as $val) {
            ...
            if ($qty_ihave < $qty) {
                ...
                $imeidata = $db->query("SELECT * FROM `imei_update` WHERE p_id = '$product_id' AND consumer_id = '$consumer_id' AND sts = 0  AND sold = 0 LIMIT $qty");
                while($imeivalue = $imeidata->fetch_object()){
                    $imei_id = $imeivalue->imei_id;
                    $imei_no = $imeivalue->imei_no;
                    $uid_no = $imeivalue->uid_no;
                    $ccid_no = $imeivalue->ccid_no;
                    $db->query("UPDATE `imei_update` SET sold = 1  WHERE imei_id = '$imei_id'");
        ...
        foreach ($imei_ids as $val) {
                ...
                $imeidata = $db->query("SELECT * FROM `imei_update` WHERE imei_id = '$imei_id'");
                while($imeivalue = $imeidata->fetch_object()){
                    $imei_id = $imeivalue->imei_id;
                    $imei_no = $imeivalue->imei_no;
                    $uid_no = $imeivalue->uid_no;
        ...
        case 'Cancel':
            ...
            $db->query("UPDATE `sale_data` SET sts = 3 WHERE so_id = '$so_id'");
            $data1 = $db->query("SELECT * FROM  `sale_order`  WHERE so_id = '$so_id'");
            $value1 = $data1->fetch_object();
            $amount_paid = $value1->paid_amount;
            $data = $db->query("SELECT * FROM `sale_products` WHERE so_id = '$so_id'");
            while($value = $data->fetch_object()){
            echo    $imei_no = $value->imei_no;
            $db->query("UPDATE `sale_products` SET sts = 3 WHERE so_id = '$so_id'");
            $db->query("UPDATE `imei_update` SET sold = 0 WHERE imei_no = '$imei_no'");
            }
            
        $db->query("INSERT INTO `balance_sheet`(`bs_id`,`a_id`,`type`,`name`,`amount`,`date`,`remark`,`create_at`)
          VALUES (NULL,'$a_id','2','Order Cancel','$amount_paid','$date','Order Cancel','$create_at')");
