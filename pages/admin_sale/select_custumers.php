...
<div class="card">
    ...
    <div class="row">
      <div class="col-md-6">
        <form action="sale_device.php" method="GET" enctype="multipart/form-data">
            <div class="form-group">
              <label for="">Select Distributor / Retailer</label>
              <select name="cs_id" class=" form-select" id="" data-allow-clear="true">
                <option value="">Select Distributor / Retailer</option>
                <?php 
                  $user_id = $_SESSION['a_id'];
                  $cs = $db->query("SELECT * FROM `admin` WHERE consumer_id = '$consumer_id' AND a_id != '$user_id' AND (a_type = 3 OR a_type = 4)");
                  while ($cs_value = $cs->fetch_object()) {
                      if($cs_value->a_type == 3){
                          $type = 'Distributor';
                      }else{
                          $type = 'Retailer ';
                ...
                ?>
                  <option value="<?=$cs_value->a_id;?>"><?=$cs_value->a_name;?> (<?=$type;?>)</option>
                <?php } ?>
              </select>
            </div>
            <div class="form-group">
      ...
      <div class="col-md-6">
          <div class="d-flex justify-content-between align-items-center row mt-4 gap-3 gap-md-0">
            <div class=" "><button class="btn btn-secondary add-new btn-primary" tabindex="0" aria-controls="DataTables_Table_0" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser"><span><i class="ti ti-plus me-0 me-sm-1 ti-xs"></i><span class="d-none d-sm-inline-block">Add New  Distributor</span></span></button></div>
  ...
  <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasAddUser" aria-labelledby="offcanvasAddUserLabel">
    <div class="offcanvas-header">
      <h5 id="offcanvasAddUserLabel" class="offcanvas-title">Add Distributor</h5>
